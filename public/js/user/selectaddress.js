document.addEventListener('DOMContentLoaded', () => {
    ...
    function closeAllMenus() {
        document.querySelectorAll('.address-dropdown.open').forEach(d => d.classList.remove('open'));
    }
    ...
    document.addEventListener('click', (e) => {
        ...
        if (!card) return;
        if (e.target.closest('.address-menu-wrapper') || e.target.closest('.modal-overlay')) {
            return;
        }
    ...
    document.addEventListener('click', (e) => {
        ...
        const typeInput = document.querySelector(`input[name="addr-type"][value="${d.type}"]`);
    ...
    if (modalSaveBtn) {
        modalSaveBtn.addEventListener('click', async () => {
            ...
            const addressLine = document.getElementById('fieldAddress').value.trim();
            const addressType = document.querySelector('input[name="addr-type"]:checked')?.value || 'home';
            ...
            const payload = { fullName, phone, pincode, city, state, addressLine, addressType, isDefault };
            const isEdit = Boolean(targetId);
            const url = isEdit ? `/user/addresses/${targetId}` : '/user/addresses';
